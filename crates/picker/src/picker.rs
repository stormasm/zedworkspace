use anyhow::Result;
use gpui::{
    actions, div, impl_actions, prelude::*, uniform_list, AnyElement, AppContext, ClickEvent,
    DismissEvent, EventEmitter, FocusHandle, FocusableView, Length, ListSizingBehavior,
    MouseButton, MouseUpEvent, Render, Task, UniformListScrollHandle, ViewContext, WindowContext,
};
use head::Head;
...
impl<D: PickerDelegate> Picker<D> {
    ...
    fn do_confirm(&mut self, secondary: bool, cx: &mut ViewContext<Self>) {
        if let Some(_update_query) = self.delegate.confirm_update_query(cx) {
    ...
    pub fn query(&self, _cx: &AppContext) -> String {
        match &self.head {
...
impl<D: PickerDelegate> Render for Picker<D> {
    fn render(&mut self, cx: &mut ViewContext<Self>) -> impl IntoElement {
        let _editor_position = self.delegate.editor_position();
